  ...
  function filterNewArrivals(filterValue) {
    ...
    fetch("http://100.64.1.100:8080/api/products")
      ...
      .then((data) => {
        ...
        const filteredProducts = data.filter(
          (p) => p.data.gender === filterValue && p.data.season === "2025Spring"
        );
        // 現在の要素を全て削除
        while (productGrid.firstChild) {
          productGrid.removeChild(productGrid.firstChild);
    ...
    for (const p of products) {
      ...
      productItem.innerHTML = `
        <div class="product discount product_filter">
          ...
          <div class="product_info">
            <h6 class="product_name">
              <a href="product/product-detail.html?productId=${p.data.id}">${p.data.name}</a>
            </h6>
            <div class="product_price">¥ ${p.data.price}</div>
          </div>
        </div>
      `;
