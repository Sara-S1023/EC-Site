jQuery(document).ready(function ($) {
  ...
  function initLineupProduct() {
    ...
    // JSON データを取得して要素を追加
    fetch("http://100.64.1.100:8080/api/products")
      .then((response) => {
        // レスポンスが成功かを確認
        if (!response.ok) {
      ...
      .then((data) => {
        ...
        for (const p of data) {
          if (keyword !== "") {
            if (
              p.data.name.toUpperCase().match(keyword.toUpperCase()) === null &&
              p.data.tag.match(keyword) === null
            ) {
              continue;
            }
          } else {
            if (
              (gender !== "" && gender !== p.data.gender) ||
              (category !== "" && category !== p.data.category)
            ) {
              continue;
            }
          }

          const oldPrice = p.data.oldPrice
            ? `
            <span>${p.data.oldPrice}</span>
          `
            : "";
          // 商品カードを作成
          const productItem = document.createElement("div");
          productItem.classList.add("product-item", p.data.gender);
            ...
            <div class="product discount product_filter">
              ...
              <div class="product_info">
                <h6 class="product_name">
                  <a href="product-detail.html?productId=${p.data.id}">${p.data.name}</a>
                </h6>
                <div class="product_price">¥ ${p.data.price}` +
            oldPrice +
            `</div>
              </div>
            </div>
